...
const updateBridge = window.jarvisUpdate;
const coreRefreshControl = $('#core-refresh');
const coreReconnectControl = $('#core-reconnect');
const updateCheckControl = $('#update-check');
const updateOpenControl = $('#update-open');
const updateDismissControl = $('#update-dismiss');
...
function setUpdateNoticeVisible(visible) {
  if (updateNotice) updateNotice.hidden = !visible;
}
...
function renderUpdateResult(result) {
  ...
  if (updateOpenControl) updateOpenControl.disabled = true;
  if (status === 'update_available') {
    const currentVersion = displayValue(result.currentVersion, 'installed version');
    const latestVersion = displayValue(result.latestVersion, 'new version');
    latestUpdateResult = result;
    setText('update-notice-title', `New version available · v${latestVersion}`);
    setText('update-notice-detail', `JARVIS Internal Test ${currentVersion} → ${latestVersion}. Download the new installer to update Electron and Core together.`);
    setText('update-status', `Update available · v${latestVersion}.`);
    if (updateOpenControl) updateOpenControl.disabled = false;
    setUpdateNoticeVisible(true);
    recordEvent('notice', `Update available: ${latestVersion}.`);
    return true;
  }
  if (status === 'up_to_date') {
    setText('update-status', `JARVIS is up to date · v${displayValue(result.currentVersion, 'current version')}.`);
    return false;
  }
  if (status === 'not_published') {
    setText('update-status', 'No Internal Test installer release is published yet.');
    return false;
  }
  if (status === 'unavailable') {
    setText('update-status', `Update check unavailable · ${displayValue(result.detail, 'not supported in this build')}`);
    return false;
  }
  throw new Error('Update check returned an unknown status.');
}

async function checkForAppUpdate({ manual = false } = {}) {
  if (updateCheckInFlight) {
    if (manual) setText('update-status', 'Update check is already running.');
    return false;
  }
  if (!updateBridge || typeof updateBridge.check !== 'function') {
    setText('update-status', 'Update checks are unavailable in this build.');
    if (manual) recordEvent('error', 'Update check bridge is unavailable.');
    return false;
  }
  updateCheckInFlight = true;
  if (updateCheckControl) updateCheckControl.disabled = true;
  setText('update-status', 'Checking for updates…');
  try {
    const result = await updateBridge.check();
    if (!result || typeof result.status !== 'string') {
      throw new Error('Update check returned an invalid result.');
    }
    return renderUpdateResult(result);
  } catch (error) {
    latestUpdateResult = null;
    setUpdateNoticeVisible(false);
    if (updateOpenControl) updateOpenControl.disabled = true;
    const detail = error.message || 'request failed';
    setText('update-status', `Update check failed · ${detail}`);
    recordEvent('error', `Update check: ${detail}`);
    return false;
  } finally {
    updateCheckInFlight = false;
    if (updateCheckControl) updateCheckControl.disabled = false;
  }
}

async function openAppUpdate() {
  if (!latestUpdateResult || latestUpdateResult.status !== 'update_available') {
    setText('update-status', 'No verified update is ready to open. Check again first.');
    return false;
  }
  if (!updateBridge || typeof updateBridge.open !== 'function') {
    setText('update-status', 'Opening the update page is unavailable in this build.');
    recordEvent('error', 'Update open bridge is unavailable.');
    return false;
  }
  const version = displayValue(latestUpdateResult.latestVersion, 'new version');
  if (updateOpenControl) updateOpenControl.disabled = true;
  setText('update-status', `Opening the Release page for v${version}…`);
  try {
    await updateBridge.open();
    latestUpdateResult = null;
    setUpdateNoticeVisible(false);
    setText('update-status', `Release page opened for v${version}. Download and run the installer when ready.`);
    recordEvent('info', `Opened the Internal Test update page for ${version}.`);
    return true;
  } catch (error) {
    const detail = error.message || 'request failed';
    setText('update-status', `Update page could not be opened · ${detail}`);
    recordEvent('error', `Open update page: ${detail}`);
    if (updateOpenControl) updateOpenControl.disabled = false;
    return false;
  }
}
...
function dismissAppUpdate() {
  if (!latestUpdateResult || latestUpdateResult.status !== 'update_available') return;
  setUpdateNoticeVisible(false);
  setText('update-status', `Update available · v${displayValue(latestUpdateResult.latestVersion, 'new version')} · hidden for this session.`);
}
...
updateCheckControl?.addEventListener('click', () => void checkForAppUpdate({ manual: true }));
updateOpenControl?.addEventListener('click', () => void openAppUpdate());
updateDismissControl?.addEventListener('click', dismissAppUpdate);
...
resize();
sync();
setTimeout(() => void checkForAppUpdate(), 1200);
